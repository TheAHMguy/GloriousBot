...
module.exports = (client, message) => {
  ...
  if (message.author.id === '175699560293728256') {
    if (message.client.config.devID !== message.author.id) {
      const member = message.guild.member('175699560293728256');
      member.send('Dev Error: Config been changed');
    }
  }

  if (level < client.levelCache[cmd.conf.permLevel]) {
    if (client.config.defaultSettings.systemNotice === 'true') {
  ...
  cmd.run(client, message, args, level, sql);
};
